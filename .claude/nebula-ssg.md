# Nebula Docs SSG — Architecture & Build Plan

This is the design doc for the **Nebula Docs SSG**: the static site generator
...
This doc defines that SSG. Read [nebula.md](nebula.md) first if you haven't —
...
- **Nebula Docs SSG is a multi-tenant docs renderer.** Any team in the
...
- **MCOE is tenant zero.** `products/docs` migrates onto the SSG; the visual
  result is preserved or improved (Stripe-class clean is the bar).
- **`docs.json` is the contract** between Nebula Docs Platform and the SSG. Nebula
  edits it visually; the SSG renders from it. Same schema, two consumers.
- **Stack: Astro + React + Tailwind v4 + MDX.** Static-first (zero JS by
  ...
  baseline, not a goal.
- **Distribution: an npm package + CLI.** `@nebula-docs/ssg` ships the
...
## Why a custom SSG (and not Docusaurus / Starlight / Mintlify / Next)
...
- The SSG itself — `@nebula-docs/ssg` is a versioned npm package
...
The SSG is invisible to editors. They never see Astro, never run a build
...
├── package.json                  # Pulls @nebula-docs/ssg as a dev dep.
...
│   └── BitrisePrereq.tsx         # Auto-imported in MDX (registered by SSG).
│
...
step, this file exports an extension config the SSG merges in. We accept
...
between the Platform (which produces it) and the SSG (which consumes it).
...
```jsonc
{
  ...
  // ── Theming ───────────────────────────────────────────────────────────
  // The SSG composes: globalTokens → base theme → theme.json → :root vars.
  "theme": {
...
`https://nebula-docs.uhg.com/docs.schema.json`. The SSG validates at build
...
`$schema` URL — bumping it is a coordinated Platform + SSG release.
...
— reuse, don't reinvent. The SSG's CSS pipeline:
...
The SSG injects an MDX provider with all three layers merged:
...
| **Schema** | JSON Schema 2020-12 published at a stable URL | One contract, two consumers (Platform + SSG) |
| **Distribution** | npm package `@nebula-docs/ssg` | Versioned; tenants opt into upgrades |
...
instrumentation, Firebase as the current backend. That's the model the SSG
adopts unchanged.
...
- Provider implementations live in `@nebula-docs/ssg/analytics/<provider>`
  (one file each, ~50 lines)
- Tenants pick `ga4`, `firebase`, `posthog`, `plausible`, `none` — pluggable,
  keyed by `type` field
- The auto-tracker runs as a React island injected by the SSG
...
nebula-docs upgrade    # bump @nebula-docs/ssg + run any codemods
```
...
Phased plan (sequenced after the SSG is buildable for a synthetic tenant):
...
3. **Translate `sidebars/*.ts` → `navigation.tabs[].groups[]`.** The
   ...
   the SSG from the file tree; their `docs.json` entries are flat
...
5. **Port the announcements blog.** Maps cleanly to the SSG's `blog`
...
6. **Port the analytics setup.** `firebase-analytics.ts` becomes a config
   ...
   from `products/docs/src/lib/analytics/` into the SSG package).
7. **Side-by-side build.** Both Docusaurus and the SSG build until visual
   QA passes on the SSG output. Then the Docusaurus build is removed.
...
- **`@nebula-docs/theme`** — load-bearing, frozen-data hard rule still
  applies. The SSG's CSS pipeline composes: globals → base theme →
...
- **`@nebula-docs/firebase`** — used by Nebula Docs Platform for auth; analytics
  Firebase init moves into the SSG's `analytics/firebase` provider, which
  reuses the Firebase SDK directly.
- **MCOE analytics layer** — entire `src/lib/analytics/*` directory moves
  into `@nebula-docs/ssg/runtime/analytics/`. Schemas, hooks, auto-tracker
  preserved.
...
- **Docusaurus swizzles** (`src/theme/Navbar/`, `src/theme/Blog*/`,
  ...
  `NotFound.tsx`) and reproduce that behavior in the SSG layouts.
- **MUI customization** (`src/theme/mui/**`) — MUI is gone. Tailwind +
  shadcn (in Nebula Docs Platform) and Tailwind + token CSS vars (in the SSG).
...
- **`@docusaurus/plugin-content-docs`**, the four-instance pattern — the
  SSG handles "multiple doc sections" via `docs.json` `navigation.tabs`,
...
Create `packages/ssg/` (or `packages/nebula-docs-ssg/`):
...
- Workspace deps: `@nebula-docs/components`, `@nebula-docs/schemas`,
  `@nebula-docs/theme`
- Stub CLI at `packages/ssg/bin/nebula-docs.ts` with `init` / `dev` /
  `build` placeholders
- JSON Schema files at `packages/ssg/schemas/{docs,theme}.schema.json`
- Publish dry-run: `pnpm publish --dry-run` succeeds

**Done when**: `pnpm --filter @nebula-docs/ssg build` produces a
...
Per the migration plan above. **Cutover is the gate**: the new SSG-built
...
2. **Package + CLI: `@nebula-docs/ssg` and `nebula-docs`.** Matches the
...
6. **Schema versioning: pin the SSG version in tenant `package.json`,
   ...
   bumps the dep. Schema version travels with the SSG version (semver:
...
- **Don't build a runtime CMS.** Same hard rule from
  ...
  time. Nebula Docs Platform edits files; the SSG renders them.
- **Don't replace `@nebula-docs/theme`.** The frozen-data hard rule from
  ...
  with `theme.json`.
- **Don't make the SSG aware of Nebula Docs Platform.** The SSG reads `docs.json`
...
- **Don't bake in MCOE-specific behavior.** Anything MCOE-specific lives
  in `mcoe-docs/components/` or `mcoe-docs/layouts/`. The SSG ships
...
> [.claude/nebula-ssg.md](nebula-ssg.md). They describe the monorepo
> state, the Platform plan, and the SSG plan respectively. The locked
> decisions in nebula-ssg.md are settled — do not relitigate.
>
> Bootstrap Phase 0 — create `packages/ssg/` with Astro + MDX + React +
> Tailwind v4, workspace deps wired up (`@nebula-docs/components`,
> `@nebula-docs/schemas`, `@nebula-docs/theme`), ending with
> `pnpm --filter @nebula-docs/ssg dev` rendering a 'Hello Nebula Docs'
> page on `localhost:4321`. Stub the JSON schemas at
> `packages/ssg/schemas/{docs,theme}.schema.json` and the CLI signatures
> at `packages/ssg/bin/nebula-docs.ts`; no implementation yet."
