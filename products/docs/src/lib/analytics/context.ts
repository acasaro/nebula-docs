 ...
 *
 * `doc_instance` maps to one of the four Docusaurus content plugins plus a few
 * well-known top-level pages (home, support, announcements). `doc_section` is
 * the first segment after the instance — useful for segmenting "release
 * management" vs. "mobile-ci" clicks without exploding cardinality.
 */
export function deriveSiteContext(pathname: string): SiteContextParams {
  ...
  const doc_instance = KNOWN_INSTANCES.has(first) ? first : 'other';
