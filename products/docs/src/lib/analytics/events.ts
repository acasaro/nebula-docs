/**
 * Typed event schema for Firebase Analytics.
 *
 * Conventions:
...
export interface SiteContextParams {
  /** Top-level instance: developers | resources | product | about | announcements | home | support | other */
  doc_instance?: string;
  /** First path segment after the instance (e.g. "release-management"). */
  doc_section?: string;
...
export type AnalyticsEventMap = {
  ...
  nav_click: SurfaceParams & SiteContextParams & {
    destination: string;
  ...
  outbound_click: SurfaceParams & SiteContextParams & {
    destination: string;
    destination_host: string;
  };

  /** Clicks on mailto: / tel: / teams: / other protocols. */
  protocol_click: SurfaceParams & SiteContextParams & {
    destination: string;
    protocol: string;
  };
  ...
  /** User switched MCOE theme (mcoe-default → uhc → optum, etc). */
  theme_switch: {
    from_theme: string;
    to_theme: string;
  };

  /** User toggled Docusaurus light/dark color mode. */
  color_mode_toggle: {
    mode: 'light' | 'dark';
  };
  ...
  custom_interaction: SurfaceParams & SiteContextParams & {
    action: string;
    [key: string]: unknown;
  };
};
