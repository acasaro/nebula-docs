...
type Palette = { bg: string; primary: string; accent: string; light: string; dot: string };

const palettes: Record<string, Palette> = {
  // Optum — original design, keep as-is
  'optum': {
    bg:      '#002677',
    primary: '#FF612B',
    accent:  '#0C55B8',
  ...
  // UHC — navy bg, teal primary, brand blue accent
  'uhc': {
    bg:      '#00184D',
    primary: '#00BED5',
    accent:  '#0C55B8',
  ...
  // Default — deep indigo bg, stripe blue primary, cyan accent
  'mcoe-default': {
    bg:      '#1a2457',
    primary: '#5469d4',
    accent:  '#3abff8',
...
export function GlossaryThumbnail() {
  ...
  const { bg, primary, accent, light, dot } = isDark ? darkPalette : (palettes[themeId] ?? palettes['mcoe-default']);
