 ...
 *
 * Attach once near the root (see ThemeContext / AnalyticsProvider). Listens
 * for:
...
function handleClick(e: MouseEvent) {
  // Only primary button, no modifier keys (let browser default handle new-tab).
  if (e.defaultPrevented) return;

  const target = e.target;
  if (!(target instanceof Element)) return;

  // Respect explicit opt-out subtrees.
  if (target.closest(`[${DATA_ATTR.ignore}]`)) return;

  // Docusaurus CodeBlock copy button — opt-in tracking without needing a swizzle.
  const copyBtn = target.closest('button.clean-btn[class*="copyButton"]') as HTMLButtonElement | null;
  if (copyBtn) {
  ...
  if (anchor) {
    ...
    const external = url && url.origin !== window.location.origin && !INTERNAL_HOSTS.includes(url.host);
  ...
  // Non-anchor surface (button, card body, etc.) — custom interaction.
...
function resolveLabel(surfaceEl: HTMLElement | null, anchor: HTMLAnchorElement | null): string | undefined {
  const fromAttr = surfaceEl?.getAttribute(DATA_ATTR.label) ?? anchor?.getAttribute(DATA_ATTR.label);
...
function inferElementType(anchor: HTMLAnchorElement | null, surface: HTMLElement | null): string | undefined {
...
function handleCodeCopy(button: HTMLButtonElement) {
  const container = button.closest('[class*="codeBlockContainer"]') as HTMLElement | null;
  const codeEl = container?.querySelector('code') ?? null;
  const text = codeEl?.textContent ?? '';
  const language = detectLanguage(container, codeEl);
  const ctx = deriveSiteContext(window.location.pathname);
  trackDeduped(
    'code_copy',
...
/**
 * Fire a `page_view` event for a given pathname. Intended to be driven by a
 * router-subscribed component inside React, not this installer.
 */
